...
param_grid = {"alpha": [1.e-9, 1.e-8, 1.e-7, 1.e-6, 1.e-5, 1.e-4, 1.e-3, 1.e-2, 0.1, 1., 10]}
...
def plot_cv(clf, params_grid, param = 'alpha'):
    params = [np.log10(x) for x in params_grid[param]]
...
lin_mod = linear_model.Lasso(clf.best_estimator_.alpha)
lin_mod.fit(X_train, y_train)
y_score = lin_mod.predict(X_test)
y_score = y_score.round(1)
print_metrics(y_test, y_score)
...
dump(lin_mod, 'LinearModel_fs_l1_cv.joblib')
